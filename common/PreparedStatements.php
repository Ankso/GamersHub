...
class Statements
{
    ...
    const SELECT_GAME_ID_BY_GENRE             = "SELECT a.id FROM games.game_data AS a, games.game_genres_relation AS b WHERE b.genre_id = ? AND b.game_id = a.id AND a.id NOT IN (SELECT game_id FROM users.user_games_relation WHERE user_id = ?) LIMIT 100";
